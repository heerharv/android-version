...
// MARK: - Profile (Public user metadata)
...
struct Profile: Identifiable, Codable, Hashable {
    ...
    init(from decoder: Decoder) throws {
        ...
        if let dateStr = try c.decodeIfPresent(String.self, forKey: .updatedAt) {
            updatedAt = parseISODate(dateStr)
        }
    }
}
...
struct Collection: Identifiable, Codable, Hashable {
    ...
    var name: String
    var createdAt: Date
    var visibility: String // "private", "public", "shared"
    ...
    init(id: UUID, userId: UUID, name: String, createdAt: Date, visibility: String = "private", shareCode: String? = nil) {
        self.id = id
        self.userId = userId
        self.name = name
        self.createdAt = createdAt
    ...
    init(from decoder: Decoder) throws {
        ...
        let dateStr = try c.decode(String.self, forKey: .createdAt)
        createdAt = parseISODate(dateStr) ?? Date()
        visibility = (try? c.decode(String.self, forKey: .visibility)) ?? "private"
...
struct CollectionMember: Identifiable, Codable, Hashable {
    var id: UUID
    var collectionId: UUID
    var userId: UUID
    var role: String // "viewer", "contributor", "owner"
    var createdAt: Date
