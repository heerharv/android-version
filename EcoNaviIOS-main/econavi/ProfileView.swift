        ...
        NavigationStack {
            List {
                ...
                Section {
                    ...
                    NavigationLink(destination: CarbonAnalyticsView()) {
                        ProfileRow(icon: "chart.bar.fill", iconColor: .green, title: "Carbon Analytics")
                    }
                ...
                Section("Network & Diagnostics") {
                    HStack {
                        ...
                        Spacer()
                        if userDataManager.isConnected == true {
                            Text("Connected")
                                .foregroundStyle(.green)
                                .bold()
                        } else if userDataManager.isConnected == false {
                            Text("Disconnected")
                                .foregroundStyle(.red)
                                .bold()
                        } else {
                    ...
                    Button {
                        Task { await userDataManager.checkConnectivity() }
                    } label: {
                        Label("Check Connection", systemImage: "antenna.radiowaves.left.and.right")
                    }

                    if let err = userDataManager.lastError, userDataManager.isConnected == false {
...
@MainActor
final class OfflineMapSearchService: NSObject, ObservableObject, MKLocalSearchCompleterDelegate {
...
// MARK: - Reports Flow (Exactly as in old version — no 'private')
...
// MARK: - Report Flow Views (Complete implementation from old file)

enum ReportPlaceIssueOption: String, CaseIterable, Identifiable {
...
enum ReportFlowDestination: Hashable {
...
// MARK: - Map and Location Services
...
struct ReportLocationMapCard: View {
    let hintText: String
...
@MainActor
final class ReportPlaceSearchService: NSObject, ObservableObject, MKLocalSearchCompleterDelegate {
    @Published var query: String = "" {
        didSet { completer.queryFragment = query }
    }
    @Published var results: [MKLocalSearchCompletion] = []
...
// MARK: - Route Issue Flow

struct RouteIssueChooseTripView: View {
...
struct RouteIssueChooseStepView: View {
...
struct IncidentTypePickerView: View {
...
// MARK: - Preferences Flow (Exactly as in old version)

enum DirectionsMode: String, CaseIterable, Identifiable {
